...
try {
  ...
  try {
    ...
    REPOSITORY = process.env.REPOSITORY || envConfig.REPOSITORY
  } catch (e) {
  ...
  REPOSITORY = process.env.REPOSITORY || envConfig.parsed.REPOSITORY
}
...
const logo =
  ENVIROMENT === "production"
    ? path.join(__dirname, "./public/img/dme-logo.png")
    : path.join(__dirname, "./public/img/dme-logo-app-qa.png")
...
if (["production", "qa"].includes(ENVIROMENT)) {
...
const createWindow = () => {
  ...
  if (!["production"].includes(ENVIROMENT)) {
